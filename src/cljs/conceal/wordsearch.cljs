...
(defn word-fits?
  ...
  posdir. A letter can be only placed in a cell either if the cell is empty or
  it already contains the right letter. The word is assumed to stay in bounds."
  [grid word posdir]
  {:pre [grid]}
  (if posdir
    (every-indexed?
...
(defn rand-posdir
  ...
  (let [dr (rand-nth [-1 0 1])
        dc (rand-nth (if (zero? dr) [-1 1] [-1 0 1]))
